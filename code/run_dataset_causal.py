"""
Causal ARIMA trend-feature construction. Identical pipeline to run_dataset.py
(same preprocessing, windowing, LSTM architecture, training protocol) except
for how the trend feature is built: instead of one MLE fit per (unit, sensor)
using that unit's entire available series (which lets parameters, though not
values, depend on cycles after t), this version refits ARIMA periodically on
an expanding window and only ever uses parameters estimated from data at or
before the cycle being predicted.

For each (unit, sensor) series y_1..y_L:
  - Fit ARIMA(order) on y_1..y_b (b = REFIT_BURNIN) as the first checkpoint.
  - For t = b+1 .. L:
      - if t - last_refit_t >= REFIT_STRIDE: refit ARIMA by full MLE on
        y_1..y_t (expanding window), record as the new current fit.
      - else: extend the current fit's Kalman state with y_t via
        `.append([y_t], refit=False)` (no re-optimization, cheap).
      - Take the one-step-ahead prediction for y_t from whichever of the
        above just happened. This prediction uses the Kalman-filtered state
        through y_{t-1} and parameters last estimated from data at or before
        the current refit checkpoint (<= t), never from y_t or later.
  - Cycles 1..b use the first fit's in-sample one-step-ahead values (a small,
    bounded-length exception: a burn-in of REFIT_BURNIN cycles, fit once,
    cannot itself be made causal at a per-cycle level without an even more
    expensive nested procedure, and is far shorter than a full run-to-failure
    series).

Usage: python3 run_dataset_causal.py FD001 [seed]
"""
import sys
import os
import json
...
OUT_DIR = f"results/causal/{FD}/seed_{SEED}"
os.makedirs(OUT_DIR, exist_ok=True)

t_start = time.time()
print(f"===== Running CAUSAL {FD} seed={SEED} (N_REGIMES={N_REGIMES}) =====")
    ...
    try:
        ...
        for t in range(b, L):
            if (t - last_refit_t) >= REFIT_STRIDE:
                ...
                cur_fit = cur_fit.append([series[t]], refit=False)
                fitted[t] = cur_fit.predict(start=t, end=t).iloc[0] if hasattr(cur_fit.predict(start=t, end=t), "iloc") else cur_fit.predict(start=t, end=t)[0]
        return fitted
    except Exception:
        n_fallback += 1
        return pd.Series(series).rolling(5, min_periods=1).mean().values


def arima_trend_features_causal(df, feature_cols, order=(1, 1, 0)):
    out = df.copy()
    ...
    for i, unit in enumerate(units):
        ...
        for col in feature_cols:
            series = g[col].values
            fitted = causal_arima_trend(series, order=order)
...
def train_lstm(X_tr, y_tr, X_val, y_val, n_features, epochs=60, batch_size=128, lr=1e-3, patience=8, tag=""):
    ...
    for epoch in range(epochs):
        ...
        if val_loss < best_val - 1e-4:
            ...
            bad_epochs += 1
        if (epoch + 1) % 10 == 0:
            print(f"[{FD}:causal:{tag}] epoch {epoch+1}/{epochs} val_rmse={val_loss**0.5:.2f}")
        if bad_epochs >= patience:
            print(f"[{FD}:causal:{tag}] early stopping at epoch {epoch+1}")
            break
...
for name, r in results.items():
    ...
    print(f"{FD} CAUSAL {name}: RMSE={rmse:.3f} MAE={mae:.3f} PHM_score={score:.1f} n={len(yt)}")
...
print(f"===== {FD} CAUSAL done in {time.time()-t_start:.1f}s =====")
